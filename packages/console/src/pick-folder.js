 ...
 * spawned via PowerShell. A 1x1 topmost owner window plus AttachThreadInput
 * lets a background process legally take the foreground, so the dialog shows
 * above the browser instead of behind it. Other platforms are not supported
 * and yield a 501 so the UI can fall back to manual path entry.
 *
 * Ported from vector-hub's battle-tested folder-picker (CommonJS).
 */
...
function parseSelectedPath(stdout) {
  const value = String(stdout === undefined || stdout === null ? '' : stdout)
    .replace(/\u0000/g, '')
    .replace(/^\uFEFF/, '')
    .trim()
    .replace(/^"|"$/g, '');
...
module.exports = { pickFolder, parseSelectedPath };
